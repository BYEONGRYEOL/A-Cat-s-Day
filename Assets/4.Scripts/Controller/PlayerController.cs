...
namespace Isometric
{
    public class PlayerController : CharacterController
    {
        ...
        {
            //공격받아 딜레이 중
            Managers.Time.SetTimer(0.5f, () => State = Enums.State.Idle );
        }
        ...
        private void Attack()
        {
            ...
            // 첫 공격
            if(State != Enums.State.Attack_1 && State != Enums.State.Attack_2 && State != Enums.State.Attack_3)
            {
                Debug.Log("1차공격");
                State = Enums.State.Attack_1;
        ...
        {
            // 이동, 뛰기, 기본, 상태일때만 되는거야
            if (State != Enums.State.Idle && State != Enums.State.Move && State!= Enums.State.Run)
                return;

            State = Enums.State.Interaction;
        ...
        {
            //idle, Move, 
            if (State != Enums.State.Idle && State != Enums.State.Move)
                return;
        ...
        {
            //맞은 상태, 공중, 에서는 안되어야해
            if (State == Enums.State.TakeDamage ||State == Enums.State.Die)
                return;
            State = Enums.State.Dodge;
        }
